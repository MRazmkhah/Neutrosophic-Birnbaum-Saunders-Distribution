# ------------------------------------------------------------------------
# Neutrosophic Parameter Estimation and Inferences for Gamma Model (Real Data 2)
# ------------------------------------------------------------------------
...
MLb<- function(x){
  n <- length(x)
  DL_a <- function(aa){ # Differentiating of L with respect to beta
    log(aa)-log(mean(x))-digamma(aa)-sum(log(x))/n
  }
  DL_a <- Vectorize(DL_a)
  a <- uniroot(DL_a,lower=0.01, upper=10)$root#, extendInt = "yes")$root
  b<-mean(x)/a
  return(b)
}
...
loglikelihood<- function(x){
  ...
  DL_a <- Vectorize(DL_a)
  a_hat <- uniroot(DL_a,lower=0.01, upper=10)$root
  b_hat <- mean(x)/a_hat
  
  sum(log(dgamma(x, shape=a_hat, scale = b_hat, log = FALSE)))
}
...
# ------------------------------------------------------------------------
F_t <- function(t,a,b){# F is Gamma CDF in t
...
KS_Montecarlo <- function(data, a, b, iterations = 5000) {
  ...
  F_t_q <-function(p) qgamma(p, shape=a, scale = b, lower.tail = TRUE, log.p = FALSE)
